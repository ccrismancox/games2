                         ...
                         TRUE, report = TRUE, ...)
{
    ...
    if(class(fitted)[2]=="egame12"){
      Penalty <- fitted$convergence$Penalty
      # if(Firth[2]){
        ...
        # if(type=="private"){
        #   cx <- function(x){
        #     tcrossprod(x[1:sum(rcols)], x[(sum(rcols)+1):(2*sum(rcols))])+
        #       tcrossprod(x[(sum(rcols)+1):(2*sum(rcols))], x[1:sum(rcols)])
        #   }
        #   
